...
func NewReadDiffusionGroupMsgSync(dataBase *db.DataBase, ws *Ws, loginUserID string, conversationCh chan common.Cmd2Value,
	joinedSuperGroupCh chan common.Cmd2Value, keyMsg *KeyMsg) *ReadDiffusionGroupMsgSync {
	p := &ReadDiffusionGroupMsgSync{DataBase: dataBase, Ws: ws, loginUserID: loginUserID, conversationCh: conversationCh,
		joinedSuperGroupCh: joinedSuperGroupCh, keyMsg: keyMsg}
	...
	for {
		...
		case cmd := <-m.joinedSuperGroupCh:
			if cmd.Cmd == constant.CmdLogout {
				...
				go func() {
					for _, v := range g {
						m.keyMsg.UpdateKeyFromSvr("", v, operationID)
					}
				}()
				if err == nil {
